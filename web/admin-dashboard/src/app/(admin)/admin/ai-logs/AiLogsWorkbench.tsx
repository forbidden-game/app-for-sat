...
export default function AiLogsWorkbench({
  maskEnabled,
  onToggleMask,
}: {
  maskEnabled: boolean;
  ...
  } = useSortable(filteredLogs, "created_at", "desc");
  ...
  const events = useMemo(() => {
    ...
    return selectedLog.events.map((event) => toRecord(event)).filter(Boolean) as Array<
  ...
  return (
    <section className="flex flex-col gap-5">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-xs font-medium text-[color:var(--ink-muted)]">Debug Workbench</div>
              ...
              sortedLogs.map((log) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-xs font-medium text-[color:var(--ink-muted)]">
                    ...
                    {log.error ? (
                      <span className="text-[color:var(--danger-strong)]">error</span>
                    ) : null}
                  </div>
                </button>
              ))
                ...
                <div className="grid gap-3">
                  ...
                  <div className="rounded-xl border border-[color:var(--border)] bg-[color:var(--surface-soft)] p-3">
                    ...
                    <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap break-words font-mono text-[11px] text-[color:var(--ink)]">
                      {maskEnabled
                        ? maskPII(selectedLog.system_prompt ?? "")
                    ...
                    {events.length === 0 ? (
                      ...
                      events.map((event, index) => {
                        ...
                        return (
                          <div
                            key={`${type}-${index}`}
                            className="flex flex-col gap-2 text-xs sm:flex-row sm:gap-3"
                          >
                            ...
                            <div className="min-w-0 flex-1 rounded-xl border border-[color:var(--border)] bg-[color:var(--surface)] px-3 py-2">
                              ...
                              <pre className="mt-1 max-h-40 overflow-auto whitespace-pre-wrap break-words font-mono text-[11px] text-[color:var(--ink)]">
                                {maskEnabled
                                  ? maskPII(serializeJson(event, 2))
                                  : serializeJson(event, 2)}
                              </pre>
                            </div>
                          </div>
                        );
