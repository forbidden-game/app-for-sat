-- Fix ambiguous prompt_version reference in request_english_grammar_analysis
...
begin
  ...
  select apc.prompt_version
    into v_prompt_version
    from public.ai_prompt_configs apc
    where apc.kind = 'english_grammar_analysis'
      and apc.status = 'published'
  ...
  if v_status <> 'done' then
    if coalesce(v_allow_enqueue, true) then
      ...
      update public.english_grammar_analyses ega
        set status = 'error',
